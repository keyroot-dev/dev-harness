...
**定義**: システムを同心円状の層に分割し、ソースコードの依存を常に内側
（高レベルの方針）へ向ける設計原則。外側の「詳細」が内側の「方針」に依存する。

**本プロジェクトでの適用**:

```
Frameworks & Drivers (cli/, repositories/)  ← 詳細・最も外側
    ↓ 内向き
Interface Adapters
    ↓ 内向き
Use Cases (use-cases/)  ←「境界インターフェース」を所有
    ↓ 内向き
Entities (entities/)  ← 心臓部
```

**各層の責務**:
- Entities: 企業全体で通用する最高レベルのビジネスルールと不変条件
- Use Cases: アプリケーション固有のビジネスルール。Repository等の境界I/Fを所有
- Interface Adapters: 外側の形式と内側の形式の相互変換
- Frameworks & Drivers: DB・FS・UI等の詳細。Use Case層のI/Fを実装する（DIP）

**メリット**:
- ビジネスルールがFW・DBから独立し、それらをFWなしで単体テストできる
- DB・フレームワークを「あとで差し替えられる詳細」に保てる
- 関心の分離による保守性向上
...
**依存関係のルール**（依存はすべて内向き）:
- ✅ Frameworks & Drivers → Use Cases（詳細がI/Fを実装してDIPで内向き依存）
- ✅ Use Cases → Entities
- ❌ Use Cases → Frameworks & Drivers（ビジネスルールが詳細に依存）
- ❌ Entities → 外側すべて（心臓部は何にも依存しない）
