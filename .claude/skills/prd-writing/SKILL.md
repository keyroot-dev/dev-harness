...
PRDの質を高めるため、以下が揃っていることが望ましいですが、**必須ではありません**。
アイデアメモが無い場合は、ユーザーとの対話でヒアリングしながらPRDを作成します。

### アイデアの壁打ち（任意）

プロダクトのアイデアについて、ユーザーがClaude Codeとの対話を通じてブラッシュアップを
済ませていると、より具体的なPRDになります。

### docs/ideas/ 配下のアイデアメモ（任意）

壁打ちや技術調査のメモが `docs/ideas/` 配下にあれば、その**全マークダウンファイル**を
参照して詳細化します（特定のファイル名には依存しません）。
...
これらのメモが無い場合は、上記の項目を対話でヒアリングしてから詳細化します。
...
まず、`docs/ideas/` 配下のアイデアメモを全て確認します（存在すれば）:

```bash
# docs/ideas/ 配下の全マークダウンを読み込む（README.md 等の説明ファイルは除外してよい）
Glob('docs/ideas/*.md') → 各ファイルを Read
```

メモが存在しない場合は、プロダクトのアイデア・課題・ターゲット・主要機能・MVP範囲を
ユーザーに対話でヒアリングします。

### 2. PRDドラフトの生成

アイデアメモ（またはヒアリング結果）の内容をもとに、テンプレートに従ってPRDを生成します。
...
受け入れ条件は **EARS（Easy Approach to Requirements Syntax）** の型に当てはめて書く。曖昧さが消え、**1条件が1テストケースにほぼ1:1で対応する**ため、仕様がそのままテストを駆動する（`/add-feature` の tasklist の「振る舞い」はここから導出される。導出先の規律は `steering` スキルのテスト駆動規律）。

| 型 | 構文 | 例 |
|---|---|---|
| 通常（常時） | システムは、常に〜すること | システムは、常にタスクをファイルに永続化すること |
| イベント駆動 | 〜したとき、システムは〜すること | ユーザーが add を実行したとき、システムはタスクを登録すること |
| 状態駆動 | 〜の間、システムは〜すること | オフラインの間、システムは変更をローカルに保留すること |
| 望ましくない挙動 | もし〜ならば、システムは〜すること | もしタスク名が空ならば、システムはエラーを表示し登録しないこと |
| オプション機能 | 〜を備える場合、システムは〜すること | 通知機能を備える場合、システムは期限24時間前に通知すること |

書き方の規律:
- 主語は「システム」に固定する（実装手段・クラス名を書かない）
- 1条件に複数の振る舞いを詰め込まない（「〜し、かつ〜」はテスト2つ分＝条件2つに割る）
- どの型にも当てはまらない条件は、要求がまだ曖昧なサイン（分解するか、非機能要件へ移す）
...
- [ ] 受け入れ条件が EARS 記法（上記「受け入れ条件（EARS 記法 / 正本）」）で測定可能に定義されているか
...
1. **アイデアメモをベースに作成**: `docs/ideas/` 配下の壁打ち内容を参照（無ければ対話でヒアリング）
