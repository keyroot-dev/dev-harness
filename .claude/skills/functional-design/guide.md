...
**クリーンアーキテクチャの例（依存は内向き）**:
```mermaid
graph TB
    User[ユーザー]
    CLI[Interface Adapters<br/>CLI / Controller]
    UseCase[Use Cases<br/>«interface» Repository を所有]
    Entities[Entities / 心臓部]
    Data[Frameworks & Drivers<br/>DB / FS]

    User --> CLI
    CLI --> UseCase
    UseCase --> Entities
    Data -. implements .-> UseCase
```

**より詳細な例（具体名つき。矢印はすべて内向き、永続化は DIP で逆転）**:
```mermaid
graph TB
    User[ユーザー]
    CLI["adapters/TaskController<br/>(Commander.js)"]
    CreateTask["use-cases/CreateTask"]
    Repo["«interface» use-cases/TaskRepository"]
    Task["entities/Task"]
    FileRepo["infrastructure/FileTaskRepository"]
    JSON[(tasks.json)]
...
各コンポーネントの責務を、クリーンアーキテクチャの輪（依存はすべて内向き）に揃えて定義します。
古典レイヤード（CLI → サービス → データ）で**並べない**こと。詳細（CLI・データ）が内側のビジネスルールに依存し、ビジネスルールは詳細を知らない（依存方向は `architecture.md` 参照）。

#### adapters（最も外側に近い）

**責務**: 外形（CLI入力）↔ 内形（ユースケース）の変換。ビジネスロジックも永続化も持たない。

```typescript
// adapters/TaskController.ts ── ユースケースを呼ぶだけ。受け取るのは DTO で Entity ではない
class TaskController {
  constructor(private readonly createTask: CreateTask) {}
  async add(title: string): Promise<void> {
    const output = await this.createTask.execute({ title }); // DTO が返る
    this.presenter.show(output);
  }
}
```

#### use-cases（アプリ固有のビジネスルール）

**責務**: 「ユーザーが何をしたいか」を1ユースケース=1クラス（動詞）で表す。包括的な `Manager`/`Service` に溜め込まない（SRP）。依存するのは**自分が所有する境界I/F**だけ。

```typescript
// use-cases/CreateTask.ts / CompleteTask.ts / ListTasks.ts … 動詞で分割する
class CreateTask {
  constructor(private readonly repository: TaskRepository) {} // 抽象に依存（実装は注入）
  async execute(input: CreateTaskInput): Promise<CreateTaskOutput> {
    const task = Task.create(TaskId.generate(), input.title); // 不変条件は Entity が守る
    await this.repository.save(task);
    return { id: task.id.value, title: task.title };          // 境界を越えるのは DTO
  }
}

// use-cases/TaskRepository.ts ── 境界I/F は use-cases が所有する（実装は infrastructure）
...
#### infrastructure（詳細 / 最も外側）

**責務**: 永続化・I/O の「詳細」。use-cases のI/Fを実装する（DIP）。依存の矢印は infrastructure → use-cases（内向き）。

```typescript
// infrastructure/FileTaskRepository.ts ── use-cases の TaskRepository を実装する
class FileTaskRepository implements TaskRepository {
  async save(task: Task): Promise<void> { /* ファイルへ書き込む */ }
  async findById(id: TaskId): Promise<Task | null> { /* ファイルから読む */ }
}
```
...
sequenceDiagram
    participant User
    participant Ctrl as adapters/TaskController
    participant UC as use-cases/CreateTask
    participant Task as entities/Task
    participant Repo as infrastructure/FileTaskRepository

    User->>Ctrl: devtask add "タスク"
    Ctrl->>Ctrl: 信頼できない入力の形だけ検証
    Ctrl->>UC: execute({ title })
    UC->>Task: Task.create(id, title)
    Note over Task: 不変条件（1〜200文字）は Entity が守る
    Task-->>UC: Task
    UC->>Repo: save(task)
    Note over UC,Repo: UC は «interface» TaskRepository を呼ぶ<br/>FileTaskRepository が DIP で実装
    Repo-->>UC: 成功
    UC-->>Ctrl: CreateTaskOutput（DTO / Entityではない）
    Ctrl-->>User: "タスクを作成しました (ID: xxx)"
```
